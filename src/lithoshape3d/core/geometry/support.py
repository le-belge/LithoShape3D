...
def _flat_foot_mesh(width_mm: float, support: PrintSupport) -> trimesh.Trimesh:
    x_min = -support.overhang_left_mm
    x_max = width_mm + support.overhang_right_mm
...
def _reinforcement_ribs(width_mm: float, support: PrintSupport) -> list[trimesh.Trimesh]:
    ...
    usable_start, usable_end = margin, max(width_mm - margin, margin)
    if usable_end <= usable_start:
        centers = [width_mm / 2.0]
    else:
    ...
    for x_center in centers:
        x_center = float(np.clip(x_center, half_thickness, width_mm - half_thickness))
...
def build_support_mesh(width_mm: float, support: PrintSupport) -> trimesh.Trimesh | None:
    """Construit le pied seul (pas encore fusionne). `None` si SupportType.NONE."""
    if support.support_type is SupportType.NONE:
        return None

    parts = [_flat_foot_mesh(width_mm, support)]
    if support.support_type is SupportType.REINFORCED:
        parts.extend(_reinforcement_ribs(width_mm, support))
...
def attach_support(mesh: trimesh.Trimesh, width_mm: float, support: PrintSupport) -> trimesh.Trimesh:
    """Fusionne le pied au modele compose (union manifold3d). Retourne `mesh`
    inchange si `support.support_type is SupportType.NONE`."""
    support_mesh = build_support_mesh(width_mm, support)
